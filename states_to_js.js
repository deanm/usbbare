    ...
    for (var w = 0, wl = pred.length; w < wl; w += 2) {
      ...
      if (name === "parse") {
        if (1||transtype === "packet") {
          code = pre + "var " + val + " = new usb_structs.Fields();\n" + code;
          expression += "usb_structs.parse_" + val + "(" + val + ", _pp.data, 0, _pp.data.length)";
        } else {
...
console.log("var usb_structs = require('./usb_structs.js');\n");
