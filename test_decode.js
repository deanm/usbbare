...
function process_packet(packet) {
  ...
  switch (group) {
    ...
    case 1:  // Token
      ...
      text += "token " + type;
      if (pid >> 2 === 1) {  // SOF
        var framenum = packet[1] | (packet[2] << 8 & 0x700);
        text += " frame: " + framenum;
      } else {
        var addr = packet[1] & 0x7f;
        var endp = (packet[1] >> 7 & 1) | (packet[2] << 1 & 0xe);
        text += " addr: 0x" + addr.toString(16) + " endp: " + endp;
      }
    ...
    case 3:  // Data
      ...
      if (next_is_setup === true) {
        ...
        for (var i = 0, il = fields.length; i < il; i += 3) {
          text += ' ' + fields[i] + ': 0x' + fields[i+1].toString(16);
          if (fields[i+2] !== null) text += ' (' + fields[i+2] + ')';
        }
        next_is_setup = false;
      }
      break;
